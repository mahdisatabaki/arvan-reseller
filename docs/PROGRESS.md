...
**قدم بعدی: T-11.3 (رهرسال دمو) — بقیه‌ی بلوک ۱۱ (T-11.4/۱۱.۵/۱۱.۶ اقدام انسانی‌اند: ضبط و تحویل).**

**تکمیل بعد از گزارش بالا:** T-10.5 (فقط مسیر بحرانی مشتری روی موبایل واقعی ۳۷۵px — CDN/ثبت‌نام/شارژ/سفارش/service-detail، بدون overflow) و T-11.2 (`arvan-reseller/README.md`، ۷ بخش، توسط یک ایجنت پس‌زمینه نوشته و توسط من بازبینی/یک اصلاح جزئی شد — ادعای نسخه‌ی وردپرس ۶.۰+ که هیچ‌جا تأیید نشده بود حذف شد) هر دو انجام شدند.
...
### بلوک ۶ — Limits + Lifecycle (شروع)

**تصمیم محصولی پیش از شروع (با کاربر):** T-6.3 طبق نوشته‌ی BACKLOG می‌گفت Suspend باید `holdResource` را صدا بزند — ولی این متد هیچ‌وقت روی `CdnClient` وجود نداشت (باز از T-1.1، عمداً حذف‌شده، نه فراموش‌شده). گزینه‌ها به کاربر ارائه شد: (الف) فقط وضعیت محلی، (ب) استفاده از `deleteResource()` به‌جای hold. کاربر (الف) را انتخاب کرد. یعنی از الان تا وقتی مکانیزم واقعی پیدا شود، Suspend سرویس CDN واقعی روی آروان را متوقف نمی‌کند — فقط `arvan_services.status` را عوض می‌کند. این باید صریحاً در دموی نهایی گفته شود.

- **T-6.1 — `ThresholdPolicy`** — `ThresholdPolicy` (DTO) + `ThresholdPolicyResolver` (فقط به `WalletRepository` وابسته). `terminate_grace_days` به‌عنوان پارامتر ساده گرفته می‌شود، نه با import کردن `ResellerSettings` (لایه‌ی WP) داخل `src/`. ۵ چک سبز (پس‌زمینه).
...
### T-7.3 تا T-7.6 — چهار صفحه‌ی باقی‌مانده‌ی مشتری (بستن بلوک ۷)

**استراتژی موازی‌سازی:** قبل از شروع، دو گپ زیرساختی مشترک که همه‌ی چهار تسک به آن نیاز داشتند خودم ساختم (`wp/Frontend/CurrentCustomer.php` برای resolve مشتری جاری + `wp/Frontend/templates/partials/topbar.php` برای نویگیشن مشترک)، بعد T-7.3 (⛔ بحرانی، بیشترین منطق دامنه) و T-7.5 (که معلوم شد همان صفحه‌ی service-detail است، نه یک صفحه‌ی جدا) را خودم پیش بردم، و T-7.4 و T-7.6 را که کاملاً از نظر فایل مستقل بودند (تمپلیت‌ها و کنترلرهای جدا، بدون همپوشانی) به دو ایجنت پس‌زمینه‌ی موازی سپردم — هر دو صریحاً از دست‌زدن به `Plugin.php`/`foundation.css`/`BACKLOG.md` منع شدند تا با نوشتن هم‌زمان روی یک فایل مشترک تداخل نکنند؛ سیم‌کشی نهایی و مستندسازی را خودم بعد از برگشتن هر دو انجام دادم.

**دو گپ معماری واقعی پیدا و رفع شد قبل از شروع UI:**
1. هیچ پورتی «همه‌ی سرویس‌های یک مشتری» (`ServiceRepository::allForCustomer`)، «تاریخچه‌ی پرداخت یک مشتری» (`PaymentRepository::historyForCustomer`) یا «تاریخچه‌ی مصرف یک مشتری» (`UsageLogRepository::historyForCustomer`) را برنمی‌گرداند — هر سه اضافه شدند (port + پیاده‌سازی WP)، همگی IDOR-safe و اسکوپ‌شده به customer_id، طبق همان الگوی موجود در `LedgerRepository::historyForCustomer`.
2. منطق «حل کلید API پیش‌فرض + رمزگشایی» فقط داخل `MeteringCronHandler::resolveClient()` (خصوصی) وجود داشت و فقط از api_key_id یک سرویس از‌قبل‌موجود کار می‌کرد — سفارش جدید (T-7.3) به همان منطق نیاز داشت ولی از نقطه‌ی شروع متفاوتی (کلید پیش‌فرض یک purpose، نه یک سرویس). استخراج شد به `wp/Arvan/CdnClientResolver.php` مشترک؛ `MeteringCronHandler` بازتوان‌بخشی شد تا از آن استفاده کند به‌جای تکرار منطق.

**T-7.3 — CDN Product Page:** `wp/Frontend/templates/cdn.php` + `wp/Frontend/Controllers/OrderController.php`. نکته‌ی معماری مهم که در همه‌ی چهار template این بلوک تکرار می‌شود: چون `template_include` فقط یک مسیر فایل خام برمی‌گرداند (نه یک شیء کنترلر با متغیرهای از‌پیش‌ساخته، برخلاف Setup Wizard)، خودِ فایل template نقش composition root صفحه را بازی می‌کند — `global $wpdb`، ساخت مستقیم adapterهای لازم، سپس `get_header()`/topbar/محتوا/`get_footer()`. سومین گپ معماری اینجا پیدا شد: `ResellerSettings::getLayout()` (cards/compact) از T-2.4 ذخیره می‌شد ولی هیچ مصرف‌کننده‌ای نداشت (DESIGN.md §۸ دقیقاً همین را پیش‌بینی کرده بود: «a default layout value is still stored... so T-7.3 has something to read from day one») — رفع شد، `cdn.php` الان واقعاً بین دو حالت چیدمان سوییچ می‌کند.

**T-7.4 — Login/Register + Recharge (ایجنت پس‌زمینه):** `AuthController`/`auth.php` + `RechargeController`/`recharge.php`. جریان Mock Payment دومرحله‌ای (initiate → confirm/fail، چون گیت‌وی واقعی وجود ندارد، ADR-014). تصمیم درست و مهم ایجنت: تأیید موفق پرداخت بلافاصله `SuspensionEngine::resumeEligibleForCustomer()` را هم صدا می‌زند — دقیقاً همان الزام ADR-012 («شارژ موفق می‌تواند سرویس معلق‌شده را ازسر بگیرد») که به‌آسانی می‌شد فراموش شود چون در بریف اولیه صریح خواسته شده بود. ثبت‌نام هرگز مستقیماً ردیف مشتری/کیف‌پول نمی‌سازد — به هوک `user_register` (T-3.3) تکیه می‌کند. Rate limiting دقیقاً به‌سبک `AccessTokenGate` (۵ تلاش/۹۰۰ ثانیه). ۲۹ چک ایجنت سبز، شامل یک تست IDOR مستقیم (مشتری B نمی‌تواند پرداخت مشتری A را تأیید کند).

**T-7.5 — Provisioning Result:** صفحه‌ی جدا ساخته نشد. چون `ProvisioningService::provision()` هم‌زمان است (بدون صف async در MVP)، لحظه‌ی ریدایرکت از OrderController وضعیت سرویس از قبل نهایی است (`active` یا `provisioning_failed`) — پس `service-detail.php` (که هم T-13 و هم T-7.5 است) با یک template واحد هر دو spec را پوشش می‌دهد. تست زنده با یک سفارش واقعی (کلید API واقعی، دامنه‌ی تستی) نشان داد `ArvanCdnClient` واقعاً فراخوانی می‌شود، Provider واقعاً شکست می‌خورد (HTTP 405، چون credential تستی معتبر نیست)، و رکورد محلی `provisioning_failed` قابل‌بازیابی می‌ماند — دقیقاً طبق الزام CLAUDE.md.

**T-7.6 — Customer Dashboard (ایجنت پس‌زمینه):** `account.php`، چهار تب سرور-رندرشده با `?tab=` (بدون جاوااسکریپت). بنر معلق‌شدن (اولویت) یا هشدار موجودی کم (`elseif`، تصمیم صریح ایجنت برای جلوگیری از دو هشدار هم‌زمان روی هم). ۳۰ چک سبز شامل جداسازی بین‌مشتری با fixture سوم. باگ جزئی خارج از scope گزارش شد (`WpPaymentRepository::markFailed()` مقدار `note` را حتی با `$reason=null` بازنویسی می‌کند) — رفع نشد، یادداشت شد.
...
### بلوک ۱۰ — Security audit (T-10.1 تا T-10.4 و T-10.6؛ Settlement/Responsive عمداً کنار گذاشته شد)

کاربر با محدودیت زمانی صریح (۱ ساعت) خواست فقط کارهای «لازم و ضروری» انجام شود. طبق فهرست قربانی خودِ DEMO.md، Settlement (بلوک ۹) و Responsive QA رسمی (T-10.5) کنار گذاشته شدند؛ ممیزی امنیتی (🔒، صراحتاً «هرگز قربانی نکن») در اولویت اول قرار گرفت.
...
~~سایت تست محلی کثیف بود~~ — **حل شد در بلوک ۱۰**: جدول‌های تراکنشی/مشتری پاک شدند، دو کاربر تستی حذف شدند؛ `arvan_api_keys` و آپشن‌های `arvan_reseller_*` عمداً دست‌نخورده ماندند. رمز ادمین محلی (`admin`) از بلوک ۸ همچنان `ArvanDemo!2026` است — قبل از تحویل نهایی باید توسط کاربر عوض/حذف شود، چون این مقدار در تاریخچه‌ی این گفتگو ثبت شده | نیاز به یادآوری کاربر قبل از انتشار/تحویل مخزن | T-2.4 → حل‌شده در T-10 |
